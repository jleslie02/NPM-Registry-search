  ...
  const classes = {
    ...
    stable: css(
      (() => ({
        ...
        color: "white",
        ...theme.mixins.boxShadow("inset -1px 0px 8px 0px #e84848"),
        "@media (max-width: 650px)": {
          marginLeft: "6px",
          marginTop: "10px"
        }
      ...
      <div css={classes.description}>
        <div css={classes.header}>
          ...
          <div css={classes.stats}>
            {Object.keys((score || {}).detail || {}).map((stat, idx) => {
              return (
                <div key={`${stat}-${data.name}`} className="statWrapper">
                  <span className={`${stat} statName`}>{stat}</span>
                  <span className="statNumber">
                    {Math.round(score.detail[stat] * 100)}
%
                  </span>
                  {idx < 2 && <div css={classes.separate} />}
                </div>
              );
            })}
          </div>
          {/* PACKAGE DESCRIPTION */}
          {data.description && (
            <div css={classes.summary}>{data.description}</div>
          )}
          {/* PACKAGE KEYWORDS */}
      ...
      <div css={classes.stableTag}>
        {Object.keys(flags || {}).map(flag => {
          return (
            <span key={`${data.name}-key-${flag}`} css={classes.stable}>
              {flag}
            </span>
          );
        })}
...
RegistryItem.propTypes = propTypes;
RegistryItem.defaultProps = defaultProps;
